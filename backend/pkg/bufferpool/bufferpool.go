...
import (
	...
	"github.com/josexy/flowlens/backend/pkg/buf"
)
...
func New(initSize int) *BufferPool[*buf.Buffer] {
	return NewPool(
		func() *buf.Buffer { return buf.NewSize(initSize) },
		func(buf *buf.Buffer) bool {
			if buf.Cap() >= MaxBufferSize {
				return false
			}
			buf.Reset()
