...
控制台使用虚拟化日志表格展示当前标签页最近一次发送的最多 1,000 条输出，列出时间、类型、来源和 Python 输出。点击日志行可在详情窗口查看完整文本，并可切换换行、复制或保存单条输出；工具栏还支持复制全部输出、保存到本地文件和清空当前输出。同一条执行链中的所有全局插件和当前请求脚本都会显示在这里，全局插件显示插件名称，当前请求脚本显示为 **当前脚本**。一次 `print()` 调用对应一条日志，输出内容中的换行会保留在该条日志内。
...
可以使用 `context.log.debug()`、`info()`、`warning()` 或 `error()`。`print()`、`sys.stdout.write()` 和 `sys.stderr.write()` 都会保留一次调用中的换行，不会按换行拆成多条日志；`print()` 和 stdout 会作为 `info` 捕获，stderr 会作为 `error` 捕获。本次执行的输出显示在 HTTP 请求编辑器的 **控制台**标签中。Python 插件工作台有意不保留另一份日志历史：规则、参数、文件和校验状态留在插件工作台，运行输出则跟随产生它的请求执行。

不要使用 `os.write(1, ...)`、`sys.__stdout__` 或让子进程继承 Worker 的 stdout 输出。它们绕过日志包装层，可能把非协议字节写入 Worker 通道并导致执行失败。`os.write(2, ...)`、`sys.__stderr__` 以及子进程继承的 stderr 也不会成为控制台日志，只会留在 Worker 的诊断 stderr 中。
